...
namespace API.Quiz.Repositories
{
    public class QuizRepository : IQuizRepository
    {
        ...
        public async Task<List<QuizDTO>> GetAsync(string userId)
        {
            ...
            return await learningLanternContext.Quiz.Where(quiz => quiz.InstructorId == userId).Select(quiz => new QuizDTO(quiz)).ToListAsync();
        ...
        public async Task<int?> AddAsync(AddQuizDTO addQuizDTO)
        {
            ...
            var quiz = await learningLanternContext.Quiz.AddAsync(new QuizModel(addQuizDTO, user));
        ...
        public async Task<bool?> UpdateAsync(QuizDTO quizDTO)
        {
            ...
            var quiz = learningLanternContext.Quiz.Update(new QuizModel(quizDTO, user));
        ...
        {
            var quiz = learningLanternContext.Quiz.Remove(new QuizModel() { Id = quizId });
